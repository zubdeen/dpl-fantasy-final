alter table public.fantasy_transfer_reset_audit
  add column if not exists lineups_before jsonb not null default '[]'::jsonb;
...
begin
  ...
  select coalesce(jsonb_agg(to_jsonb(l) order by l.period, l.player_id), '[]'::jsonb)
  into v_lineups_before
  from public.fantasy_manager_lineups l
  where l.manager_id = p_manager_id
    and l.season = 'Season 05';
  ...
  if p_restore_squad then
    ...
    select exists (
      select 1 from public.fantasy_transfer_events later
  ...
  insert into public.fantasy_transfer_reset_audit(
    ...
    reason, from_player_id, to_player_id, squad_before, lineups_before
  ) values (
    auth.uid(), p_manager_id, v_event.id, v_event.season, v_event.period, p_restore_squad,
    v_reason, v_event.from_player_id, v_event.to_player_id, v_squad_before, v_lineups_before
  );
